...
const log = debug('app:square-accounts-dynamo');
 ...
 *
 * Env: `SQUARE_ACCOUNTS_TABLE_NAME`, `SQUARE_ACCOUNTS_ACCOUNT_KEY_GSI` (default `account-key`).
 */
const tableName = () => process.env.SQUARE_ACCOUNTS_TABLE_NAME;

const accountKeyGsiName = () => process.env.SQUARE_ACCOUNTS_ACCOUNT_KEY_GSI || 'account-key';
...
const findFirstItemByAccountKey = async (TableName, account_key) => {
    ...
    if (collected.length > 1) {
        console.warn(
            'square-accounts: multiple items share account_key; using first match',
            account_key
        );
    }
    return collected[0];
...
const putSquareAccount = async (item) => {
    const TableName = tableName();
    if (!TableName) {
        console.warn('SQUARE_ACCOUNTS_TABLE_NAME not set; skip DynamoDB put');
        return;
    }

    if (item?.id == null || String(item.id).trim() === '') {
        throw new Error('putSquareAccount: id is required');
    }
    if (item?.account_key == null || String(item.account_key).trim() === '') {
        throw new Error('putSquareAccount: account_key is required');
    }
    ...
    if (existing) {
        ...
        if (partitionId == null || String(partitionId).trim() === '') {
            throw new Error(
                'putSquareAccount: existing item matched by account_key is missing id (partition key)'
            );
        }
...
const scanAllSquareAccounts = async () => {
    const TableName = tableName();
    if (!TableName) {
        throw new Error('SQUARE_ACCOUNTS_TABLE_NAME is not configured');
    }
    const acc = [];
    let ExclusiveStartKey;
    do {
 ...
 */
const deleteSquareAccountsByAccountKey = async (account_key) => {
    const TableName = tableName();
    if (!TableName) {
        console.warn('SQUARE_ACCOUNTS_TABLE_NAME not set; skip DynamoDB delete');
        return;
    }
    if (account_key == null || String(account_key).trim() === '') {
        throw new Error('deleteSquareAccountsByAccountKey: account_key is required');
    }
    ...
    for (const item of items) {
        ...
        if (id == null || String(id).trim() === '') {
            console.warn('square-accounts: skip delete for item without id (partition key)', ak);
            continue;
        }
